...
// but consider: could you do this search more quickly? (and what would have to
// be true for you to do that? ...)
...
char **load_vocabulary(char *filename, size_t *num_words) {
    ...
    char **vocabulary = malloc((*num_words) * sizeof(char *));
    if (vocabulary == NULL) {
        fclose(f);
        return NULL;
    }
    ...
    for (size_t i = 0; i < *num_words; i++) {
        vocabulary[i] = malloc(6 * sizeof(char));  // 5 letters + '\0'
